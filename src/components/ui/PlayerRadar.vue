...
// Score chiffré, posé sur un anneau extérieur fixe (au-delà du polygone, en
// deçà de l'intitulé) : toujours visible et bien détaché de l'intitulé.
// Couleur du rouge (faible) au vert (élevé) selon la valeur.
...
const values = computed(() => props.axes.map((a, i) => {
  ...
  const u = unit(i)
  const anchor = Math.abs(u.x) < 0.3 ? 'middle' : (u.x > 0 ? 'start' : 'end')
  return { x: p.x, y: p.y, anchor, value: a.value, color: valueColor(a.value) }
}))
</script>
...
<template>
  <svg :viewBox="`0 0 ${SIZE} ${SIZE}`" width="100%"
    ...
    <text v-for="(val, i) in values" :key="'n' + i" :x="val.x" :y="val.y" :text-anchor="val.anchor"
      dominant-baseline="middle" font-size="12" font-weight="700" :fill="val.color"
